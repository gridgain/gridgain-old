...
public class GridP2PHotRedeploymentSelfTest extends GridCommonAbstractTest {
    ...
    private void processTestHotRedeployment(GridDeploymentMode depMode) throws Exception {
        try {
            ...
            int[] res1 = grid1.compute().execute(taskCls, Collections.singletonList(grid2.localNode().id())).get();
            ...
            int[] res2 = (int[])grid1.compute().execute(taskCls.getName(),
                Collections.singletonList(grid2.localNode().id())).get();
...
//            }, EVT_TASK_UNDEPLOYED);
            ...
            int[] res1 = grid1.compute().execute(taskCls1, Collections.singletonList(grid2.localNode().id())).get();

            assert res1 != null;

            info("Result1: " + Arrays.toString(res1));

            int[] res2 = grid1.compute().execute(taskCls2, Collections.singletonList(grid2.localNode().id())).get();
